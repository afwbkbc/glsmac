...
OpenGLGraphics::OpenGLGraphics( const string title, const unsigned short window_width, const unsigned short window_height, const bool vsync, const float fov) {
	m_window = NULL;
	m_gl_context = NULL;
...
void OpenGLGraphics::AddScene( scene::Scene *scene ) {
...
void OpenGLGraphics::RemoveScene( scene::Scene *scene ) {
...
void OpenGLGraphics::UpdateCamera() {

}

void OpenGLGraphics::LoadTexture( const types::Texture* texture ) {
...
void OpenGLGraphics::UnloadTexture( const types::Texture* texture ) {
	Log("Unloading texture '" + texture->m_name + "'");
	glDeleteTextures(1, &m_textures[texture] );
}
*/

void OpenGLGraphics::EnableTexture( const types::Texture* texture ) {
	if ( texture ) {
		glBindTexture( GL_TEXTURE_2D, m_textures[texture] );
	}
	else {
...
void OpenGLGraphics::DisableTexture() {
	glBindTexture( GL_TEXTURE_2D, 0 );
}
