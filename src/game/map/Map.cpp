	...
	: m_game( game )
{
	...
	{ // prepare tile states
		module_pass.clear();
		NEW( m, module::Prepare, this ); module_pass.push_back( m ); // needs to always be first
	...
	{ // needs to be in separate pass because moisture original textures need to be available for all tiles in next pass
		module_pass.clear();
		NEW( m, module::LandMoisture, this ); module_pass.push_back( m );
		m_modules.push_back( module_pass );
	}
	
	{ // main pass
		module_pass.clear();
		NEW( m, module::LandSurface, this ); module_pass.push_back( m );
		NEW( m, module::WaterSurface, this ); module_pass.push_back( m );
		NEW( m, module::CalculateCoords, this ); module_pass.push_back( m );
		NEW( m, module::Coastlines1, this ); module_pass.push_back( m );
		m_modules.push_back( module_pass );
	}
	
	{ // second pass. needed to read and write tile states of tile or neighbors after they are fully generated in main pass
		module_pass.clear();
		NEW( m, module::Coastlines2, this ); module_pass.push_back( m );
		m_modules.push_back( module_pass );
	}
	
	{ // finalize and write vertices to mesh
		module_pass.clear();
		NEW( m, module::Finalize, this ); module_pass.push_back( m );
		m_modules.push_back( module_pass );
	}
	...
	{
		module_pass.clear();
		NEW( m, module::LandSurfacePP, this ); module_pass.push_back( m );
		m_modules_deferred.push_back( module_pass );
	}
	{
		module_pass.clear();
		NEW( m, module::WaterSurfacePP, this ); module_pass.push_back( m );
		NEW( m, module::Sprites, this ); module_pass.push_back( m );
		m_modules_deferred.push_back( module_pass );
	}
}
...
const Map::error_code_t Map::Initialize( MT_CANCELABLE ) {
	...
	m_tiles->Validate( MT_C ); MT_RETIFV( EC_ABORTED );
	...
	m_map_state->LinkTileStates( MT_C ); MT_RETIFV( EC_ABORTED );
	
	InitTextureAndMesh(); MT_RETIFV( EC_ABORTED );
	
	// some processing is only done on first run
	m_map_state->first_run = true;
	
	const auto tiles = m_tiles->GetVector( MT_C ); MT_RETIFV( EC_ABORTED );
	
	LoadTiles( tiles, MT_C ); MT_RETIFV( EC_ABORTED );
	
	m_meshes.terrain->Finalize(); MT_RETIFV( EC_ABORTED );
	m_meshes.terrain_data->Finalize(); MT_RETIFV( EC_ABORTED );
	
	FixNormals( tiles, MT_C ); MT_RETIFV( EC_ABORTED );
...
void Map::LoadTiles( const tiles_t& tiles, MT_CANCELABLE ) {
	...
	ProcessTiles( m_modules, tiles, MT_C ); MT_RETIF();
	...
	ProcessTiles( m_modules_deferred, tiles, MT_C ); MT_RETIF();
}
...
void Map::FixNormals( const tiles_t& tiles, MT_CANCELABLE ) {
	...
	for ( const auto& tile : tiles ) {
		...
		m_meshes.terrain->SetVertexNormal( ts->layers[ TileState::LAYER_LAND ].indices.center, (
