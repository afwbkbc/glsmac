...
MoraleSet::MoraleSet( const std::string& id, const morale_values_t& morale_values )
	...
	ASSERT( MORALE_MIN == 0, "morale min should be zero for now" );
	ASSERT( morale_values.size() == MORALE_MAX - MORALE_MIN + 1, "morale set expects exactly " + std::to_string( MORALE_MAX - MORALE_MIN + 1 ) + " values" );
}
