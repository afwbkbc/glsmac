...
void Prepare::GenerateTile( const Tile* tile, Map::tile_state_t* ts, Map::map_state_t* ms ) {
	...
	for ( auto lt = 0 ; lt < Map::LAYER_MAX ; lt++ ) {
		...
		ts->layers[ lt ].tex_coords = {
			{ ts->tex_coord.x, ts->tex_coord.y },
			{ ts->tex_coord.x1, ts->tex_coord.y2 },
			{ ts->tex_coord.x1, ts->tex_coord.y1 },
			{ ts->tex_coord.x2, ts->tex_coord.y1 },
			{ ts->tex_coord.x2, ts->tex_coord.y2 }
		};
