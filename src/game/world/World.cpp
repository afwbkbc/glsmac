...
const Buffer World::Serialize() const {
...
const Buffer World::sprite_actor_t::Serialize() const {
	Buffer buf;
...
void World::Unserialize( Buffer buf ) {
...
void World::sprite_actor_t::Unserialize( Buffer buf ) {
	name = buf.ReadString();
	tex_coords = buf.ReadVec2u();
	z_index = buf.ReadFloat();
}

const std::string& World::GetErrorString( const error_code_t& code ) {
...
Tile* World::GetTile( const size_t x, const size_t y ) const {
	ASSERT( m_tiles, "tiles not set" );
	return m_tiles->At( x, y );
}

TileState* World::GetTileState( const size_t x, const size_t y ) const {
	ASSERT( m_map_state, "map state not set" );
	return m_map_state->At( x, y );
}

TileState* World::GetTileState( const Tile* tile ) const {
	// TODO: link by pointer?
	return GetTileState( tile->coord.x, tile->coord.y );
}

const MapState* World::GetMapState() const {
	ASSERT( m_map_state, "map state not set" );
	return m_map_state;
}

void World::ClearTexture() {
	ASSERT( m_map_state, "map state not set" );
...
void World::AddTexture( const TileState::tile_layer_type_t tile_layer, const Consts::pcx_texture_coordinates_t& tc, const Texture::add_flag_t mode, const uint8_t rotate, const float alpha, util::Perlin* perlin ) {
	ASSERT( m_map_state, "map state not set" );
...
void World::CopyTextureFromLayer( const TileState::tile_layer_type_t tile_layer_from, const size_t tx_from, const size_t ty_from, const TileState::tile_layer_type_t tile_layer, const Texture::add_flag_t mode, const uint8_t rotate, const float alpha, util::Perlin* perlin ) {
	ASSERT( m_map_state, "map state not set" );
...
void World::CopyTexture( const TileState::tile_layer_type_t tile_layer_from, const TileState::tile_layer_type_t tile_layer, const Texture::add_flag_t mode, const uint8_t rotate, const float alpha, util::Perlin* perlin ) {
...
void World::CopyTextureDeferred( const TileState::tile_layer_type_t tile_layer_from, const size_t tx_from, const size_t ty_from, const TileState::tile_layer_type_t tile_layer, const Texture::add_flag_t mode, const uint8_t rotate, const float alpha, util::Perlin* perlin ) {
	ASSERT( m_map_state, "map state not set" );
...
void World::GetTexture( Texture* dest_texture, const Consts::pcx_texture_coordinates_t& tc, const Texture::add_flag_t mode, const uint8_t rotate, const float alpha ) {
...
void World::GetTextureFromLayer( Texture* dest_texture, const TileState::tile_layer_type_t tile_layer, const size_t tx_from, const size_t ty_from, const Texture::add_flag_t mode, const uint8_t rotate, const float alpha ) const {
	ASSERT( m_map_state, "map state not set" );
	ASSERT( dest_texture->m_width == s_consts.tc.texture_pcx.dimensions.x, "tile dest texture width mismatch" );
	ASSERT( dest_texture->m_height == s_consts.tc.texture_pcx.dimensions.y, "tile dest texture height mismatch" );
	dest_texture->AddFrom(
...
void World::SetTexture( const TileState::tile_layer_type_t tile_layer, TileState* ts, Texture* src_texture, const Texture::add_flag_t mode, const uint8_t rotate, const float alpha ) {
	ASSERT( m_map_state, "map state not set" );
...
void World::SetTexture( const TileState::tile_layer_type_t tile_layer, Texture* src_texture, const Texture::add_flag_t mode, const uint8_t rotate, const float alpha ) {
	SetTexture( tile_layer, m_current_ts, src_texture, mode, rotate, alpha );
}

const World::tile_texture_info_t World::GetTileTextureInfo( const texture_variants_type_t type, const Tile* tile, const tile_grouping_criteria_t criteria, const uint16_t value ) const {
	ASSERT( m_current_ts, "GetTileTextureInfo called outside of tile generation" );
	World::tile_texture_info_t info;
...
Random* World::GetRandom() const {
	return m_game->GetRandom();
}

const size_t World::GetWidth() const {
	ASSERT( m_tiles, "tiles not set" );
	return m_tiles->GetWidth();
}

const size_t World::GetHeight() const {
	ASSERT( m_tiles, "tiles not set" );
	return m_tiles->GetHeight();
}

Tiles* World::GetTilesPtr() const {
	ASSERT( m_tiles, "tiles not set" );
	return m_tiles;
}

const std::string World::GetTerrainSpriteActor( const std::string& name, const Consts::pcx_texture_coordinates_t& tex_coords, const float z_index ) {
...
const size_t World::AddTerrainSpriteActorInstance( const std::string& key, const Vec3& coords ) {
...
void World::RemoveTerrainSpriteActorInstance( const std::string& key, const size_t instance_id ) {
	ASSERT( m_sprite_actors.find( key ) != m_sprite_actors.end(), "actor not found" );
...
const World::error_code_t World::Generate(
#ifdef DEBUG
	...
	Vec2< size_t > size = map_settings.size == MapSettings::MAP_CUSTOM
		...
		: world::s_consts.map_sizes.at( map_settings.size );
#ifdef DEBUG
	util::Timer timer;
	timer.Start();
...
const World::error_code_t World::LoadFromBuffer( Buffer buffer ) {
	if ( m_tiles ) {
		DELETE( m_tiles );
	}
	NEW( m_tiles, Tiles );
...
const World::error_code_t World::LoadFromFile( const std::string& path ) {
...
const Buffer World::SaveToBuffer() const {
	return m_tiles->Serialize();
}

const World::error_code_t World::SaveToFile( const std::string& path ) const {
	try {
...
const World::error_code_t World::Initialize( MT_CANCELABLE ) {
...
void World::ProcessTiles( module_passes_t& module_passes, const tiles_t& tiles, MT_CANCELABLE ) {
	ASSERT( m_map_state, "map state not set" );
...
void World::LoadTiles( const tiles_t& tiles, MT_CANCELABLE ) {
...
void World::FixNormals( const tiles_t& tiles, MT_CANCELABLE ) {
	Log( "Fixing normals" );
...
void World::CalculateTextureVariants( const texture_variants_type_t type, const texture_variants_rules_t& rules ) {
