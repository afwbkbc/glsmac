...
#undef x
	...
	if ( tile->features & Tile::F_JUNGLE ) {
		auto txinfo = m_map->GetTileTextureInfo( World::TVT_TILES, tile, World::TG_FEATURE, Tile::F_JUNGLE );
		m_map->AddTexture(
			TileState::LAYER_LAND,
	...
	if ( !tile->is_water_tile ) {
		...
		if ( tile->terraforming & Tile::T_FOREST ) {
			auto txinfo = m_map->GetTileTextureInfo( World::TVT_RIVERS_FORESTS, tile, World::TG_TERRAFORMING, Tile::T_FOREST );
		...
		if ( tile->features & Tile::F_RIVER ) {
			auto txinfo = m_map->GetTileTextureInfo( World::TVT_RIVERS_FORESTS, tile, World::TG_FEATURE, Tile::F_RIVER );
