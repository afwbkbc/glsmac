...
#define X( _b, _l, _t, _c ) \
    NEW( m_buttons._b, ::ui::object::Button, "PopupButtonCenterArea" ); \
    m_buttons._b->SetLeft( _l ); \
    m_buttons._b->SetLabel( _t ); \
    m_buttons._b->On( ::ui::event::EV_BUTTON_CLICK, EH( this ) { \
...
void CenterArea::ActivateButton( ::ui::object::Button* button ) {
#define X( _b, _c ) \
    if ( m_buttons._b == button ) { \
        m_buttons._b->AddStyleModifier( ::ui::M_SELECTED ); \
        m_pages._b->Show(); \
    } \
    else { \
        m_buttons._b->RemoveStyleModifier( ::ui::M_SELECTED ); \
        m_pages._b->Hide(); \
    }
	X( resource, Resource )
	X( support, Support )
