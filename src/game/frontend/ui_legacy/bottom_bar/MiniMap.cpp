...
void MiniMap::Create() {
	...
	NEW( m_map_surface, ::ui_legacy::object::Mesh, "BBMinimapImage" );
	...
	m_map_surface->On(
		::ui_legacy::event::EV_MOUSE_DOWN, EH( this, f_scrollto ) {
			m_is_dragging = true;
			f_scrollto(
				{
	...
	m_map_surface->On(
		::ui_legacy::event::EV_MOUSE_MOVE, EH( this, f_scrollto ) {
			if ( m_is_dragging ) {
				f_scrollto(
					{
						data->mouse.relative.x,
	...
	m_map_surface->On(
		::ui_legacy::event::EV_MOUSE_SCROLL, EH( this, f_scrollto ) {
			f_scrollto(
				{
					data->mouse.relative.x,
					data->mouse.relative.y
	...
	NEW( m_bottom_bar, ::ui_legacy::object::Section, "BBMinimapBottomBar" );
	AddChild( m_bottom_bar );
	NEW( m_bottom_bar_labels.mission_year, ::ui_legacy::object::Label, "BBMinimapBottomBarLabel" );
	...
	NEW( m_bottom_bar_labels.energy, ::ui_legacy::object::Label, "BBMinimapBottomBarLabel" );
...
void MiniMap::SetMinimapSelection( const types::Vec2< float > position_percents, const types::Vec2< float > zoom ) {
	...
	if ( size != m_last_selection_size || !m_map_selection ) {
		...
		NEW( m_map_selection, ::ui_legacy::object::Mesh );
		m_map_selection->SetAlign( ::ui_legacy::ALIGN_LEFT | ::ui_legacy::ALIGN_TOP );
