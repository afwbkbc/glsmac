...
void Toolbar::Create() {
	...
	{ // tool buttons
		...
		uint8_t bx = 0, by = 0;
		for ( auto t = backend::map_editor::TT_NONE + 1 ; t < backend::map_editor::TT_MAX ; t++ ) {
			const auto tool = (backend::map_editor::tool_type_t)t;
			ASSERT( m_tool_names.find( tool ) != m_tool_names.end(), "tool name not found" );
			const std::string& tool_name = m_tool_names.at( tool );
			NEWV( button, ::ui_legacy::object::SimpleButton, "BBMiddleAreaToolbarButton Tool " + tool_name );
			button->SetLeft( offset.x + bx * s_tool_button_width );
			button->SetTop( offset.y + by * s_tool_button_height );
			button->On(
				::ui_legacy::event::EV_MOUSE_DOWN, EH( this, tool ) {
					SelectTool( tool );
					return true;
				}
			);
	...
	{ // brush buttons
		...
		uint8_t bx = 0, by = 0;
		for ( auto b = backend::map_editor::BT_NONE + 1 ; b < backend::map_editor::BT_MAX ; b++ ) {
			const auto brush = (backend::map_editor::brush_type_t)b;
			ASSERT( m_brush_names.find( brush ) != m_brush_names.end(), "brush name not found" );
			const std::string& brush_name = m_brush_names.at( brush );
			NEWV( button, ::ui_legacy::object::SimpleButton, "BBMiddleAreaToolbarButton Brush " + brush_name );
			button->SetLeft( offset.x + bx * s_brush_button_width );
			button->SetTop( offset.y + by * s_brush_button_height );
			button->On(
				::ui_legacy::event::EV_MOUSE_DOWN, EH( this, brush ) {
					SelectBrush( brush );
					return true;
				}
			);
	...
	{ // tool info
		...
		for ( auto t = TI_NONE + 1 ; t < TI_MAX ; t++ ) {
			...
			NEWV( label, ::ui_legacy::object::Label, "BBMiddleAreaToolbarInfoLabel" );
	...
	SelectTool( backend::map_editor::TT_ELEVATIONS );
	SelectBrush( backend::map_editor::BT_DOT );
}

void Toolbar::Destroy() {

	{ // tools
		SelectTool( backend::map_editor::TT_NONE );
	...
	{ // brushes
		SelectBrush( backend::map_editor::BT_NONE );
...
void Toolbar::Align() {
	...
	if ( m_tool_info.body ) {
		...
		if ( m_object_area.width >= 460 ) {
			...
			m_centered_block->SetAlign( ::ui_legacy::ALIGN_CENTER );
		}
		else {
			m_tool_info.body->Hide();
			m_centered_block->SetAlign( ::ui_legacy::ALIGN_LEFT );
		}
	}
}
...
void Toolbar::SelectTool( backend::map_editor::tool_type_t tool ) {
	...
	if ( m_game->GetEditorTool() != tool ) {
		...
		if ( m_active_tool_button ) {
			m_active_tool_button->RemoveStyleModifier( ::ui_legacy::M_SELECTED );
		}
		if ( tool == backend::map_editor::TT_NONE ) {
			m_active_tool_button = nullptr;
		}
		else {
			m_active_tool_button = m_tool_buttons.at( tool - 1 ); // because there's no button for backend::map_editor::TT_NONE
			m_active_tool_button->AddStyleModifier( ::ui_legacy::M_SELECTED );
		}
...
void Toolbar::SelectBrush( backend::map_editor::brush_type_t brush ) {
	...
	if ( m_game->GetEditorBrush() != brush ) {
		...
		if ( m_active_brush_button ) {
			m_active_brush_button->RemoveStyleModifier( ::ui_legacy::M_SELECTED );
		}
		if ( brush == backend::map_editor::BT_NONE ) {
			m_active_brush_button = nullptr;
		}
		else {
			m_active_brush_button = m_brush_buttons.at( brush - 1 ); // because there's no button for backend::map_editor::BT_NONE
			m_active_brush_button->AddStyleModifier( ::ui_legacy::M_SELECTED );
		}
