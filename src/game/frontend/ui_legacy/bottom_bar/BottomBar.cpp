...
#include "game/frontend/ui_legacy/bottom_bar/left_menu/LeftMenu.h"
#include "game/frontend/ui_legacy/bottom_bar/right_menu/RightMenu.h"
#include "ObjectPreview.h"
#include "TilePreview.h"
#include "MiddleArea.h"
#include "game/frontend/ui_legacy/bottom_bar/objects_list/ObjectsList.h"
#include "MiniMap.h"
...
void BottomBar::Create() {
	...
	NEW( m_buttons.menu, ::ui_legacy::object::Button, "BBButtonMenu" );
	m_buttons.menu->SetLabel( "MENU" );
	m_buttons.menu->On(
		::ui_legacy::event::EV_BUTTON_CLICK, EH( this ) {
			if ( m_side_menus.left->IsVisible() ) {
				m_buttons.menu->RemoveStyleModifier( ::ui_legacy::M_SELECTED );
				m_side_menus.left->Hide();
			}
			else {
				m_buttons.menu->AddStyleModifier( ::ui_legacy::M_SELECTED );
	...
	NEW( m_buttons.commlink, ::ui_legacy::object::Button, "BBButtonCommlink" );
	m_buttons.commlink->SetAlign( ::ui_legacy::ALIGN_TOP | ::ui_legacy::ALIGN_RIGHT );
	m_buttons.commlink->SetLabel( "COMMLINK" );
	m_buttons.commlink->On(
		::ui_legacy::event::EV_BUTTON_CLICK, EH( this ) {
			if ( m_side_menus.right->IsVisible() ) {
				m_buttons.commlink->RemoveStyleModifier( ::ui_legacy::M_SELECTED );
				m_side_menus.right->Hide();
			}
			else {
				m_buttons.commlink->AddStyleModifier( ::ui_legacy::M_SELECTED );
	...
	NEW( m_message_label, ::ui_legacy::object::Label, "BBMessageLabel" );
...
void BottomBar::CloseMenus() {
	if ( m_buttons.menu->HasStyleModifier( ::ui_legacy::M_SELECTED ) ) {
		m_buttons.menu->RemoveStyleModifier( ::ui_legacy::M_SELECTED );
		m_side_menus.left->Hide();
	}
	if ( m_buttons.commlink->HasStyleModifier( ::ui_legacy::M_SELECTED ) ) {
		m_buttons.commlink->RemoveStyleModifier( ::ui_legacy::M_SELECTED );
		m_side_menus.right->Hide();
	}
}
