...
CLASS( RenderMesh, Mesh )
	...
	static const uint8_t VERTEX_SIZE = Mesh::VERTEX_COORD_SIZE + VERTEX_TEXCOORD_SIZE + VERTEX_TINT_SIZE + VERTEX_NORMAL_SIZE;
	static const uint8_t SURFACE_SIZE = 3; // triangles

	RenderMesh( const size_t vertex_count, const size_t surface_count );
	
	index_t AddVertex( const Vec3 &coord, const Vec2<coord_t> &tex_coord = { 0, 0 }, const Color tint = { 1.0f, 1.0f, 1.0f, 1.0f }, const Vec3 &normal = { 0.0f, 0.0f, 0.0f } );
	index_t AddVertex( const Vec2<coord_t> &coord, const Vec2<coord_t> &tex_coord = { 0, 0 }, const Color tint = { 1.0f, 1.0f, 1.0f, 1.0f }, const Vec3 &normal = { 0.0f, 0.0f, 0.0f } );
