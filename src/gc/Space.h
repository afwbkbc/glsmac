...
CLASS( Space, common::Class )
	...
	void Accumulate( gc::Object* const owner, const f_accum_t& f, const f_accum_t& f_cleanup = nullptr, const bool need_now = false );
...
private:
	...
	std::shared_mutex m_collect_mutex;
	...
	std::shared_mutex m_pending_accumulations_mutex; // callbacks from other threads will be deferred and executed on Iterate()
	struct accum_info_t {
