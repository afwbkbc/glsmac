...
void UI::AddObject( object::UIObject* object ) {
	...
	if ( object->HasEventContext( ui_legacy::object::UIObject::EC_OFFCLICK_AWARE ) ) {
...
void UI::TriggerGlobalEventHandlers( global_event_handler_order_t order, ui_legacy::event::UIEvent* event ) {
...
const event::UIEventHandler* UI::AddGlobalEventHandler( const ui_legacy::event::event_type_t type, const event::handler_function_t& func, const global_event_handler_order_t order ) {
...
void UI::Confirm( const std::string& text, const ui_handler_t on_confirm ) {
	...
	popup->SetText( text );
	popup->On(
		ui_legacy::event::EV_CONFIRM, EH( on_confirm ) {
			on_confirm();
			return true;
		}
	);
