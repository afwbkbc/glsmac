...
void MenuBlock::Create() {
	...
	for ( size_t i = m_items.size() ; i > 0 ; i-- ) {
		...
		menu_item->On(
			ui_legacy::event::EV_MOUSE_MOVE, EH( this, item, i ) {
	...
	On(
		ui_legacy::event::EV_KEY_DOWN, EH( this ) {
			if ( !data->key.modifiers ) {
				if ( data->key.code == ui_legacy::event::K_UP ) {
					if ( m_selected_item_index < m_items.size() - 1 ) {
						SetActiveItem( m_selected_item_index + 1 );
					}
				}
				else if ( data->key.code == ui_legacy::event::K_DOWN ) {
				...
				else if ( data->key.code == ui_legacy::event::K_ENTER ) {
	...
	NEW( m_sound, ui_legacy::object::SoundEffect, "SlidingMenuSound" );
	AddChild( m_sound );
...
void MenuBlock::SetActiveItem( const size_t index ) {
	if ( index != m_selected_item_index || !m_selected_item ) {
		...
		if ( item != m_selected_item ) {
			if ( m_selected_item ) {
				m_selected_item->RemoveStyleModifier( ui_legacy::M_SELECTED );
			}
			item->AddStyleModifier( ui_legacy::M_SELECTED );
