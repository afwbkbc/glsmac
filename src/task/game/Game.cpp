	...
	if ( m_mt_ids.init ) {
		...
		if ( response.result != ::game::R_NONE ) {
			...
			switch ( response.result ) {
				...
				case ::game::R_ERROR: {
					ui->GetError()->Show( *response.data.error.error_text, UH( this ) {
						if ( m_on_cancel ) {
							m_on_cancel();
						}
					});
		...
		if ( response.result != ::game::R_NONE ) {
			...
			if( response.result == ::game::R_SUCCESS ) {
				...
				ui->GetError()->Show( "Map saving failed.", UH( this ) {
...
void Game::LoadMap( const std::string& path ) {
	...
	auto* game = g_engine->GetGame();
	g_engine->GetUI()->GetLoader()->Show( "Loading game", LCH( this ) {
		CancelRequests();
		return false;
	});
...
void Game::ResetMapState() {
	...
	Vec2< size_t > coords = { m_map_data.width / 2, m_map_data.height / 2 };
	if ( ( coords.y % 2 ) != ( coords.x % 2 ) ) {
		coords.y++;
	}
	GetTileAtCoords( { coords.x, coords.y } );
}
