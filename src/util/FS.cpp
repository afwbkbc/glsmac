...
std::vector< std::string > FS::ListDirectory( const std::string& directory, const bool return_absolute_paths ) {
	...
	try {
		...
		for (const std::string& item : items ) {
			ASSERT_NOLOG( item.substr( 0, directory.size() + 1 ) == directory + "/", "unexpected path in directory list results: " + item );
			result.push_back( return_absolute_paths ? item : item.substr( directory.size() + 1 ) );
		}
